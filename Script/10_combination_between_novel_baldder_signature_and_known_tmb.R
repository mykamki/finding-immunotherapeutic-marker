...
bar_plot_by_combination <- function(dataset) {
	...
	hh <- sum(data[data$Combination_Signature_TMB == "HH",]$n)
	ll <- sum(data[data$Combination_Signature_TMB == "Any",]$n)

	data$tt <- c(rep(hh,nrow(data[data$Combination_Signature_TMB == "HH",])), 
		     rep(ll,nrow(data[data$Combination_Signature_TMB == "Any",])))
	data <- data %>% mutate(percent = round(n/tt*100,1)) 
	...
	apply(dt[,2:3], 2, change_na2zero) %>% fisher.test(simulate.p.value=TRUE,B=1e7) -> dtt
        ...
        p <- ggplot(data, aes(fill=response.binary, y=percent, x=Combination_Signature_TMB)) +
                geom_col() + 
                geom_signif(comparisons = list(c("HH", "Any")), 
                            annotations = paste0("pvalue=", round(dtt$p.value,2)), y_position = 102) +
                scale_x_discrete(labels=c(paste0("HH\n(n=", hh,")"), 
					  paste0("Any\n(n=", ll,")"))) +
                scale_fill_manual(values = c("orange", "royalblue3")) +
...
load(paste0(indir,"log_dataset_gse176307.RData"))
load(paste0(indir,"log_dataset_imvigor210core.RData"))
load(paste0(indir,"log_dataset_ucgenome.RData"))
...
qa1 <- my_HH_survival_plot(clinical_gse176307_2, "")
qa2 <- my_LH_survival_plot(clinical_gse176307_2, "")
qa3 <- my_HL_survival_plot(clinical_gse176307_2, "")

qb1 <- my_HH_survival_plot(clinical_imvigor210core_2, "")
qb2 <- my_LH_survival_plot(clinical_imvigor210core_2, "")
qb3 <- my_HL_survival_plot(clinical_imvigor210core_2, "")

qc1 <- my_HH_survival_plot(clinical_ucgenome_2, "")
qc2 <- my_LH_survival_plot(clinical_ucgenome_2, "")
qc3 <- my_HL_survival_plot(clinical_ucgenome_2, "")
